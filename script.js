...
function initPlayer(url) {
    ...
    if (location.protocol === 'https:' && url.startsWith('http:')) {
        console.error('Mixed Content Error: Cannot play HTTP stream on HTTPS site.');
        showError(
            'Güvenlik Hatası (Mixed Content)',
            'Siteniz HTTPS (Güvenli) ancak yayın linki HTTP (Güvenli Değil). Tarayıcı bu bağlantıyı engelledi.',
            'Çözüm: Yayın linkini HTTPS yapın veya tarayıcıda "Güvenli olmayan içeriğe izin ver" seçeneğini açın.'
        );
        return;
    }
