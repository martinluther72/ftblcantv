...
import { getFirestore, collection, addDoc, onSnapshot, query, orderBy, limit, serverTimestamp, getDocs, deleteDoc, doc, getDoc, setDoc, updateDoc, increment } from "https://www.gstatic.com/firebasejs/10.13.0/firebase-firestore.js";
...
function waitForReady() {
    // Check if HLS is loaded and DOM is ready
    if (typeof Hls !== 'undefined' && document.getElementById('stream-player')) {
        ...
        if (initAttempts < maxAttempts) {
            ...
            console.error('Failed to initialize: Dependencies not loaded');
        }
    }
}
...
function initApp() {
    ...
    // Initialize chat after a short delay to ensure DOM is fully loaded
    setTimeout(() => {
        initChat();
    }, 100);
...
function loadAdminSettings() {
    ...
    const chatChannelText = localStorage.getItem('chatChannelText');
    const chatChannelLink = localStorage.getItem('chatChannelLink');
    ...
    if (streamCategory) {
        const categoryEl = document.getElementById('stream-category-display');
        if (categoryEl) categoryEl.textContent = streamCategory;
    }

    if (chatChannelText) {
        const chatTextEl = document.getElementById('chat-channel-text');
        if (chatTextEl) chatTextEl.textContent = chatChannelText;
    }

    if (chatChannelLink) {
        const chatBtnEl = document.getElementById('chat-channel-btn');
        if (chatBtnEl) chatBtnEl.href = chatChannelLink;
    }
}
...
function initChat() {
    ...
    if (!chatMessages || !chatForm || !chatInput) {
        console.error('Chat elements not found:', { chatMessages, chatForm, chatInput });
        return;
    }
    ...
    function initEmojiPicker() {
        ...
        emojiPicker.innerHTML = emojis.map(emoji =>
            `<button type="button" class="emoji-item hover:bg-gray-700 p-1 rounded text-xl transition-colors">${emoji}</button>`
        ).join('');
    ...
    function renderMessage(id, data) {
        ...
        const badge = data.badge || ''; // Get badge from message data
                ...
                try {
                    ...
                    if (userSnap.exists()) {
                        ...
                        if (userData.isBanned) {
                            alert("Bu kanaldan banlandınız. Mesaj gönderemezsiniz.");
                            return;
                        }
                        ...
                        await setDoc(userRef, {
                            messageCount: 1,
                            isBanned: false,
                            firstMessageTime: serverTimestamp(),
                            lastMessageTime: serverTimestamp()
                        });
                    }
                    ...
                    // Determine badge
                    let badge = null;
                    if (currentUsername === 'Admin' || currentUsername === 'admin') badge = '🛡️';
                    else if (messageCount >= 1000) badge = '🥇';
                    else if (messageCount >= 200) badge = '🥈';
                    else if (messageCount >= 100) badge = '🥉';
                    ...
                    if (snapshot.size > 30) {
                        // Delete oldest messages (keep newest 30)
                        const messagesToDelete = [];
                        snapshot.docs.slice(30).forEach(doc => {
                            messagesToDelete.push(deleteDoc(doc.ref));
                        });
...
`;
    ...
    setInterval(() => {
        ...
        const headerBannerImg = document.getElementById('header-banner-img');
        const headerBannerAnchor = document.getElementById('header-banner-link');

        if (headerBanner && headerBannerImg && headerBannerAnchor) {
            if (headerActive && headerUrl) {
                headerBanner.classList.remove('max-h-0', 'opacity-0', 'mb-0');
                headerBanner.classList.add('max-h-32', 'opacity-100', 'mb-4');
                if (headerBannerImg.src !== headerUrl) headerBannerImg.src = headerUrl;
                if (headerBannerAnchor.href !== headerLink) headerBannerAnchor.href = headerLink;
            } else {
                headerBanner.classList.add('max-h-0', 'opacity-0', 'mb-0');
                headerBanner.classList.remove('max-h-32', 'opacity-100', 'mb-4');
            }
        }
    }, 1000);
}
